...
In Basics we'll be learning Git so that we can keep track of different versions of our code. In [8.2 (GitHub Repo Browsing)](../8-github/7.2-github-repo-browsing.md) we'll cover how to browse and get back these older versions, but in this section we'll only cover some basic actions within Git and how to record the versions of our code in the first place.
...
{% hint style="info" %}
Eventually our list of commits will be longer than the terminal window height, and `git log` will open a special menu to navigate this list. To exit this menu, use `q`, as suggested in [1.3.: Command Line's Troubleshooting section](../1-introduction/2.1-command-line.md#troubleshooting-the-command-line).
{% endhint %}
