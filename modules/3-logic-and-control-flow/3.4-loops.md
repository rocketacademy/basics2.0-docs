...
We can also next for loops to represent more than 2 dimensions. Conventionally, instead of using the `counter` variable, `i, j, k` are used to communicate clearly and succinctly the dimension the loops are at. Typically, 2 dimensions of loops is sufficient., anymore than 3 nested for loops or more is rare.&#x20;
...
\
The code below if the [Nested Loops to Simulate Dimensions](3.4-loops.md#nested-loops-to-simulate-dimensions) example written in for loops, the code here is much shorter and cleaner as the variables are internalised in the for-loop statements.
