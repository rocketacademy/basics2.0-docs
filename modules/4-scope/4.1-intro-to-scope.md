...
* `let` is used to declaring variables in block scope. Variables declared with `let` are no longer available outside the block.
* `const` is used to declare **constants.**  It is similar to `let` in that variables are declared in block scope.&#x20;
